...
export class ProductionKernel {
  public constructor(
    ...
    private readonly config: Pick<ProductionConfig, "effectAllowedHosts">,
  ...
  private async search(
    ...
    >(
      `WITH RECURSIVE reachable(entity_id, depth) AS (
       ...
       scored AS (
         SELECT
           ...
           GREATEST(
             0,
             1 - (assertion.embedding <=> $1::vector)
         ...
         WHERE assertion.system_from <= $3
           ...
           AND ($5::TEXT IS NULL OR graph_match.graph_distance IS NOT NULL)
      ...
      [
        ...
        systemAt,
        validAt,
        operation.relatedToEntityId ?? null,
        operation.maxGraphDepth ?? 2,
        operation.predicate ?? null,
        operation.kind ?? null,
        operation.perspective ?? null,
        operation.limit ?? 20,
      ],
    );
